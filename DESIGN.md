...
### Shadow Vocabulary
- **Rest** (`0 1px 2px rgba(27,23,23,0.04), 0 10px 30px rgba(27,23,23,0.06)`): a permanent, slight drop shadow - the lift of a plate hung on a wall, not a hover cue. Carried by the About page's framed hero artwork, and (2026-09-09) by every product-artwork image container - grid/listing cards, the PDP hero and thumbnail rail, cart line items, the checkout order summary, and the homepage tile rails - so the art reads as mounted on the Cotton wall rather than printed flat onto the page. Corners on these containers stay radius 0 (Cut-Edge Rule; a considered rounded-corner "metal plate" treatment was tried and rejected 2026-09-09 as too far from the steel identity) - the shadow alone carries the framed-on-a-wall read. Admin's own upload-management thumbnails are left flat (no shadow) - the quiet product register, not a storefront expression.
