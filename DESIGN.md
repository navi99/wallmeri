...
| section | heading |
|---|---|
| hero | EVERY WALL DESERVES *a story.* |
...
Depth is **almost entirely absent**, and that is the point. In a gallery the wall is continuous; only the art is raised. Cards, panels, and form fields sit flat on the canvas, held by a Line hairline and nothing else. Structural depth comes free from the light/dark architecture: the Noir header and footer bound the Cotton wall without needing shadows at all. Interaction may earn `shadow-lift`; rest almost never earns `shadow-card` - the one deliberate exception is product-artwork image containers (see below), which rest in `shadow-card` everywhere by design.
...
### Shadow Vocabulary
- **Rest** (`0 6px 10px rgba(27,23,23,0.08), 0 22px 45px rgba(27,23,23,0.12)`; x stays 0 on both layers so it falls straight down, never sideways): a permanent, slight drop shadow - the lift of a plate hung on a wall, not a hover cue. Carried by the About page's framed hero artwork, and (2026-09-09) by every product-artwork image container - grid/listing cards, the PDP hero and thumbnail rail, cart line items, the checkout order summary, and the homepage tile rails - so the art reads as mounted on the Cotton wall rather than printed flat onto the page. Corners on these containers stay radius 0 (Cut-Edge Rule; a considered rounded-corner "metal plate" treatment was tried and rejected 2026-09-09 as too far from the steel identity) - the shadow alone carries the framed-on-a-wall read. Admin's own upload-management thumbnails are left flat (no shadow) - the quiet product register, not a storefront expression.
- **Frame** (`-5px 8px 12px rgba(27,23,23,0.10), -14px 26px 45px rgba(27,23,23,0.14)`; x goes *negative* on both layers): the resting shadow of a storefront art tile - see The Hung-Print Rule below. The same gallery light, caught by a print hung on the wall rather than a plate cut flush into it, so it throws left and down instead of true south. It stays put through the hover tilt. Scoped to those art tiles; no other surface uses it.
...
### Named Rules
**The Earned-Lift Rule.** `shadow-lift` appears only in response to interaction (hover, drag, active dialog). Nothing rests in a lifted state. (This governs `shadow-lift` specifically; product-artwork's permanent `shadow-card` is a separate, named exception - see Shadow Vocabulary above - not a breach of this rule.)

**The Hung-Print Rule (2026-09-12).** Every art tile in the storefront grids and the homepage bands is a *hung print*, not a plate cut flush into the wall: a **2px Noir bezel** at the image edge - Noir Black `ink`, never #000 - and `shadow-frame` thrown left and down. At rest every print hangs **square** - the wall is level. Hovering tilts it a **uniform 2° clockwise** and lifts it over its neighbours; the tilt is the interaction, a piece nudged off-level as you reach for it, and the shadow does not change. This covers the Gallery/Category/Artist product cards, the `/artists` portrait grid, and all three homepage tile rails - and nothing else: PDP heroes, cart line items, checkout summaries, panels, dialogs and every admin surface stay level. The tilt is **uniform** - one angle for every card, never randomised per item, and it only ever appears on hover, so a resting grid stays a level wall rather than a scatter. The bezel is the only border an art tile ever gets, so the Flat-Wall Rule below is untouched (a tile has art, a bezel and a shadow - no fill). Implementation lives in one place, the `hungPrint` token in `components/custom/media-rail.tsx`; the paired 0.97 scale there is geometry, not styling - it keeps the rotated bounding box inside its grid track so no gutter had to widen.

**The Flat-Wall Rule.** A card does not get a fill *and* a border *and* a shadow. On a light surface it gets a Line hairline and the canvas fill - that's it. Reach for `shadow-card` only when a surface genuinely floats above the page (a dropdown panel, a modal, or - the scoped exception above - a product-artwork image container standing in for a hung plate), never to make a static panel "pop".
...
- **Product card:** a 3:4 artwork tile wearing the hung-print frame (2px Noir bezel and `shadow-frame` at rest, tilting 2° over 300ms on hover - updated 2026-09-12, replacing the earlier bare tile with a 6px hover lift), then title, price, artist byline, and star rating, all center-aligned beneath it (updated 2026-09-06; previously title and price shared a left/right baseline row). Title uses the shared `railLabel` token (Montserrat 400, 14px, 0.03em, warms to Premium Red on hover) so the Gallery/Category/Artist grid and the homepage tile rail read as one typographic treatment - see Tile rail below. Price sits on its own line in Premium Red directly beneath the title. The 3:4 ratio is a deliberate divergence from the reference's 1:1 - posters are portrait.
...
- **Tile rail (homepage, 2026-07-25):** the shared geometry (`MediaRail`) behind Shop by Category, Featured Pieces, and Featured Artists, so the three bands read as one gallery wall instead of three card treatments. A centered heading sits inside `container-page`; beneath it a **full-bleed** row of tiles runs off both edges of the viewport, separated by a hairline gutter (`clamp(8px,1vw,14px)`, used as both the gap and the outer inset). Tiles carry the hung-print frame and nothing more - a 2px Noir bezel, `shadow-frame`, a 2° tilt on hover, no card and no overlay (updated 2026-09-12; they were bare before) - and the label sits on the surface *beneath* the tile as a plaque that warms to Premium Red on hover. Every plaque holds a **name** (product title, category, artist), set in the shared `railLabel` token - Montserrat 400, 14px, 0.03em, centered, warming to Premium Red on hover - not the uppercase `.label` voice (sized down from 16px, 2026-09-06, when the Gallery/Category/Artist product card adopted this same token so the two surfaces stay one treatment). The same product appears in this rail and in the Gallery grid; one string must not get two treatments. Artwork tiles hold `aspect-[3/4]`; category and artist tiles use a fixed `clamp(300px,30vw,440px)` height so proportions stay gallery-like at any column count. Columns are `auto-fit minmax(200px,1fr)`, capped per rail (category 440px, artwork 380px, artist 340px per tile) so a short row centers instead of stretching tiles into landscape crops. Hover zooms the *media* 1.05× inside a frame whose cut edges never move; `motion-reduce` drops the zoom. Below `md` the same markup renders as a plain `grid-cols-2` grid (updated 2026-09-06) - same tiles, same gutter, but the row scrolls with the page instead of horizontally, matching the Gallery/Category/Artists listing convention; this retired the earlier snap-scroll rail (`overflow-x-auto` + scroll-snap on a fixed `w-[min(68vw,300px)]` tile, scrollbar hidden via `.no-scrollbar`), which bled tiles off the right edge as the mobile affordance. This also retires the treatment these three bands wore before the frame: bare tiles with `shadow-card` at rest, a whole-tile 6px hover lift, the red `SpotlightCard` glow over artwork, and the round artist avatars (artist tiles are now radius-0 portraits; a portrait-less artist gets their initial as a Cormorant italic monogram on `cream/5`).
...
- **Don't** tilt anything that isn't a storefront art tile, don't tilt one at rest, and don't vary the angle card to card - the Hung-Print Rule is one uniform 2° on hover, applied through the shared `hungPrint` token, never a per-item random rotation.
