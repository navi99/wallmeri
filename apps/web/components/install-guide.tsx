...
export function InstallGuide() {
  return (
    <section className="mt-14">
      ...
      <div className="mt-6 grid gap-px overflow-hidden border border-line bg-ink/10 sm:grid-cols-2 lg:grid-cols-4">
        {STEPS.map((step) => (
          <div key={step.n} className="bg-paper p-5">
            <span className="font-display text-4xl italic text-brand-600">{step.n}</span>
            <h3 className="mt-2 label text-ink">{step.title}</h3>
            <p className="mt-2 text-sm leading-relaxed text-muted">{step.body}</p>
          </div>
        ))}
      </div>

      <div className="flex items-start gap-3 border border-t-0 border-line bg-paper p-5">
